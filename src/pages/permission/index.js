...
export default class Order extends React.Component {
  ...
  requestList = async () => {
    const res = await axios.ajax("/role/list", {});
    let list = res.item_list.map((item, i) => {
      item.key = i;
      return item;
    });
    this.setState({ list });
  ...
  // 角色创建
  handleRole = () => this.setState({ isRoleVisible: true });
  // 角色提交
  handleRoleSubmit = async () => {
    let data = this.roleForm.props.form.getFieldsValue();
    await axios.ajax("/role/create", { ...data });
  ...
  handlePermission = () => {
    if (!this.state.selectedItem) {
      return Modal.info({ title: "信息", content: "请选择一个角色" });
    }
    this.setState({
      isPermVisible: true,
    ...
    this.setState({ menuInfo: menuList });
  };
  handlePermEditSubmit = async () => {
    let data = this.roleForm.props.form.getFieldsValue();
    data.role_id = this.state.selectedItem.id;
    data.menus = this.state.menuInfo;
    await axios.ajax("/permission/edit", { ...data });
  ...
  getRoleUserList = async id => {
    const res = await axios.ajax("/role/user_list", { id });
  ...
  patchUserInfo = targetKeys => this.setState({ targetKeys: targetKeys });
  handleUserSubmit = async () => {
    let data = {};
    data.user_ids = this.state.targetKeys || [];
    data.role_id = this.state.selectedItem.id;
    await axios.ajax("/role/user_role_edit", { ...data });
...
class RoleAuthForm extends React.Component {
  ...
  handleChange = targetKeys => this.props.patchUserInfo(targetKeys);
  render() {
    const formItemLayout = {
      labelCol: { span: 5 },
      wrapperCol: { span: 18 }
