...
export default class User extends React.Component {
  ...
  requestList = async () => {
    const res = await axios.ajax("/table/list1", { page: this.params.page });
    this.setState({
  ...
  handleOperator = type => {
    ...
    if (type === "create") {
      ...
      if (!item) {
        return Modal.info({ title: "信息", content: "请选择一个用户" });
      }
      this.setState({
      ...
      Modal.confirm({
        content: "确定要删除此用户吗？",
        onOk: async () => {
          await axios.ajax("/user/delete", { id: item.id });
  ...
  handleSubmit = async () => {
    ...
    await axios.ajax(type === "create" ? "/user/add" : "/user/edit", {
      ...data
    });
    this.setState({ isVisible: false });
    this.requestList();
      ...
      <div>
        ...
        <Modal
          ...
          onCancel={() => {
            ...
            this.setState({ isVisible: false, userInfo: "" });
          }}
        >
          <UserForm
